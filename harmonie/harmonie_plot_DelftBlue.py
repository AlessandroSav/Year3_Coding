...
for exp in exps:  
    ...
    # calc resolved tendencies 
    harm3d[exp]['utend_res'] = -harm3d[exp]['uw'].differentiate('z') * 3600
    harm3d[exp]['vtend_res'] = -harm3d[exp]['vw'].differentiate('z') * 3600
...
for var in ['u','v']:
    # wind
    harm2d[exp][var+'_on_conv']  = harm3d[exp][var+'a'].where(filtered[exp].sel(klp=5)['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'_on_dive']  = harm3d[exp][var+'a'].where(filtered[exp].sel(klp=5)['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'_off_smoc'] = harm3d[exp][var+'a'].where(filtered[exp].sel(klp=5)['smoc']==0).sel(z=slice(0,4000)).mean(('x','y'))
    # resolved flux
    harm2d[exp][var+'flx_res_on_conv']  = harm3d[exp][var+'w'].where(filtered[exp].sel(klp=5)['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'flx_res_on_dive']  = harm3d[exp][var+'w'].where(filtered[exp].sel(klp=5)['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'flx_res_off_smoc'] = harm3d[exp][var+'w'].where(filtered[exp].sel(klp=5)['smoc']==0).sel(z=slice(0,4000)).mean(('x','y'))
    # param flux
    harm2d[exp][var+'flx_conv_on_conv']  = harm3d[exp][var+'flx_conv'].where(filtered[exp].sel(klp=5)\
                                                                             ['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'flx_conv_on_dive']  = harm3d[exp][var+'flx_conv'].where(filtered[exp].sel(klp=5)\
                                                                             ['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'flx_conv_off_smoc'] = harm3d[exp][var+'flx_conv'].where(filtered[exp].sel(klp=5)\
                                                                             ['smoc']==0).sel(z=slice(0,4000)).mean(('x','y'))
    # resolved tendency
    harm2d[exp][var+'tend_res_on_conv'] = harm3d[exp][var+'tend_res'].where(filtered[exp].sel(klp=5)\
                                                                              ['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'tend_res_on_dive'] = harm3d[exp][var+'tend_res'].where(filtered[exp].sel(klp=5)\
                                                                              ['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'tend_res_off_smoc']= harm3d[exp][var+'tend_res'].where(filtered[exp].sel(klp=5)\
                                                                              ['smoc']==0).sel(z=slice(0,4000)).mean(('x','y'))
    # param tendency
    harm2d[exp][var+'tend_conv_on_conv'] = harm3d[exp][var+'tend_conv'].where(filtered[exp].sel(klp=5)\
                                                                              ['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'tend_conv_on_dive'] = harm3d[exp][var+'tend_conv'].where(filtered[exp].sel(klp=5)\
                                                                              ['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y'))
    harm2d[exp][var+'tend_conv_off_smoc']= harm3d[exp][var+'tend_conv'].where(filtered[exp].sel(klp=5)\
                                                                              ['smoc']==0).sel(z=slice(0,4000)).mean(('x','y'))
###########################
##
...
for exp in exps:
    ...
    sel_time =[np.datetime64('2020-01-07T22'),np.datetime64('2020-01-17T12'),np.datetime64('2020-02-11T05')]
    harm3d[exp].sel(time=sel_time).to_netcdf(my_harm_dir+exp+'/'+exp[16:]+'harm3d_snapshots.nc', compute=True)
##############################
##############################
...
for idx, var in enumerate(['utend_conv','vtend_conv']):
    ...
    harm3d[exp][wind_var].where(filtered[exp].sel(klp=5)['smoc_conv']>0).sel(z=slice(10,4000)).mean(('x','y','time'))\
            .plot(y='z',lw=2,ax=axs[idx,0],c='orangered',label=r'$D_{sc}>0$')
    harm3d[exp][wind_var].where(filtered[exp].sel(klp=5)['smoc_dive']>0).sel(z=slice(10,4000)).mean(('x','y','time'))\
            .plot(y='z',lw=2,ax=axs[idx,0],c='royalblue',label=r'$D_{sc}<0$')
    harm3d[exp][wind_var].where(filtered[exp].sel(klp=5)['smoc']==0).sel(z=slice(10,4000)).mean(('x','y','time'))\
    ...
    harm3d[exp][flx_var].where(filtered[exp].sel(klp=5)['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y','time'))\
            .plot(y='z',lw=2,ax=axs[idx,1],c='orangered',label=r'$D_{sc}>0$')
    harm3d[exp][flx_var].where(filtered[exp].sel(klp=5)['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y','time'))\
            .plot(y='z',lw=2,ax=axs[idx,1],c='royalblue',label=r'$D_{sc}<0$')
    harm3d[exp][flx_var].where(filtered[exp].sel(klp=5)['smoc']==0).sel(z=slice(0,4000)).mean(('x','y','time'))\
    ...
    (harm3d[exp][var].where(filtered[exp].sel(klp=5)['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y','time'))\
        /harm3d[exp][wind_var].where(filtered[exp].sel(klp=5)['smoc_conv']>0).sel(z=slice(0,4000)).mean(('x','y','time')))\
            .plot(y='z',lw=2,ax=axs[idx,2],c='orangered',label=r'$D_{sc}>0$')
    (harm3d[exp][var].where(filtered[exp].sel(klp=5)['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y','time'))\
        /harm3d[exp][wind_var].where(filtered[exp].sel(klp=5)['smoc_dive']>0).sel(z=slice(0,4000)).mean(('x','y','time')))\
            .plot(y='z',lw=2,ax=axs[idx,2],c='royalblue',label=r'$D_{sc}<0$')
    (harm3d[exp][var].where(filtered[exp].sel(klp=5)['smoc']==0).sel(z=slice(0,4000)).mean(('x','y','time'))\
        /harm3d[exp][wind_var].where(filtered[exp].sel(klp=5)['smoc']==0).sel(z=slice(0,4000)).mean(('x','y','time')))\
...
for idx, var in enumerate(['u','v']):
    ...
    #         .plot(y='z',lw=1.5,ax=axs[idx,0],c='k',label='Mean')
    harm2d[exp][var].sel(z=slice(10,4000)).mean('time')\
            .plot(y='z',lw=2,ax=axs[idx,0],c='orangered',label=r'$D_{sc}>0$')
    harm2d[exp][var].sel(z=slice(10,4000)).mean('time')\
            .plot(y='z',lw=2,ax=axs[idx,0],c='royalblue',label=r'$D_{sc}<0$')
    harm2d[exp][var].sel(z=slice(10,4000)).mean('time')\
    ...
    #         .plot(y='z',lw=1.5,ax=axs[idx,1],c='k',label='Mean')
    harm2d[exp][var+'_flx_on_conv'].sel(z=slice(0,4000)).mean('time')\
            .plot(y='z',lw=2,ax=axs[idx,1],c='orangered',label=r'$D_{sc}>0$')
    harm2d[exp][var+'_flx_on_dive'].sel(z=slice(0,4000)).mean('time')\
            .plot(y='z',lw=2,ax=axs[idx,1],c='royalblue',label=r'$D_{sc}<0$')
    harm2d[exp][var+'_flx_off_smoc'].sel(z=slice(0,4000)).mean('time')\
            .plot(y='z',lw=3,ax=axs[idx,1],c='olive',label=r'NoSMOC')
    ...
    #     /harm2d[wind_var].sel(z=slice(0,4000)).mean(('x','y','time')))\
    #         .plot(y='z',lw=1.5,ax=axs[idx,2],c='k',label='Mean')
    (harm2d[exp][var+'tend_conv_on_conv'].sel(z=slice(0,4000)).mean('time')\
        /harm2d[exp][var].sel(z=slice(0,4000)).mean('time'))\
            .plot(y='z',lw=2,ax=axs[idx,2],c='orangered',label=r'$D_{sc}>0$')
    (harm2d[exp][var+'tend_conv_on_dive'].sel(z=slice(0,4000)).mean('time')\
        /harm2d[exp][var].sel(z=slice(0,4000)).mean('time'))\
            .plot(y='z',lw=2,ax=axs[idx,2],c='royalblue',label=r'$D_{sc}<0$')
    (harm2d[exp][var+'tend_conv_off_smoc'].sel(z=slice(0,4000)).mean('time')\
        /harm2d[exp][var].sel(z=slice(0,4000)).mean('time'))\
            .plot(y='z',lw=3,ax=axs[idx,2],c='olive',label=r'NoSMOC')
    ##
    axs[idx,1].set_yticks([])
...
axs[idx,2].set_xlabel(r'$hour^{-1}$')
axs[0,0].legend(fontsize=14)
plt.savefig(figure_dir+'wind_prof_subsamp_norm_by_wind_meanofscenes.pdf')
